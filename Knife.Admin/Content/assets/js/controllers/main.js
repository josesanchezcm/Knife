...
.controller('MainController', function ($scope, $rootScope, $http, $timeout, $location/**, cfpLoadingBar*/) {
	...
	$scope.ace = $scope.ace || {};
	$scope.ace.path = {
		'assets': '../Content/assets' //used in page templates when linking to images, etc
	};
	...
	$rootScope.getData = function(dataName, type) {
		...
		if(type == 'page') {
			...
			dataPath = 'Content/assets/data/pages/'+pageName+'/'+dataName+'.json';
		}
		else {
			dataKey = type+'-'+dataName;
			dataPath = 'Content/assets/data/'+type+'/'+dataName+'.json';
		}
...
app.service('SidebarList', function() {
	...
	this.getList = function(uiStateList) {
	  ...
	  var sidebar = {'root': []};//let's start with root and call it root! (see Content/assets/views/layouts/default/partial/sidebar.html)
